#!/usr/bin/env python3
"""
maro 채널 정확한 3분 TTS 음성 생성기 (완벽 버전)
- 정확히 3분(180초)에 맞는 스크립트 생성
- 자연스러운 말하기 속도로 3분 완성
"""
...
class PerfectThreeMinuteTTSGenerator:
    def __init__(self):
        # API 키 직접 설정 (임시)
    ...
    def generate_perfect_3min_script(self):
        """정확히 3분이 되는 스크립트 생성 (600-650자)"""
        
        # 정확히 3분에 맞는 스크립트 (600-650자)
        script = """
안녕하세요, 여러분! '마음위로'에 오신 것을 환영합니다. 오늘은 여러분의 자존감을 높이는 방법에 대해 이야기해보려고 해요.

자존감은 우리의 삶을 어떻게 살아가느냐에 큰 영향을 미치는데요, 함께 알아볼까요?

첫 번째로, 작은 목표를 설정해보세요. 매일 아침, 자신에게 작은 목표를 하나 정해보는 겁니다. 예를 들어, '오늘은 물을 두 잔 마시자' 또는 '오늘은 친구에게 안부 인사를 하자' 같은 작은 것들 말이에요. 이런 작은 목표들을 달성할 때마다 자신에게 '잘했어'라고 말해주세요.

두 번째로, 자신의 강점을 찾아보세요. 우리는 종종 자신의 부족한 점에만 집중하곤 하는데, 사실 모든 사람에게는 고유한 강점이 있습니다. 예를 들어, 친구들이 고민을 털어놓을 때 잘 들어주는 것, 요리를 잘하는 것, 아니면 꾸준히 운동하는 것까지도 모두 강점이에요.
...
"""
    ...
    def generate_tts(self, text, filename="comfort_perfect_3min_narration"):
        """정확한 3분 TTS 음성 생성"""
        try:
...
def main():
    ...
    generator = PerfectThreeMinuteTTSGenerator()
    
    # 1. 정확한 3분 스크립트 생성
    print("📝 정확한 3분 스크립트 생성 중...")
    script_content = generator.generate_perfect_3min_script()
    
    # 2. 스크립트 저장
    script_path = os.path.join(generator.output_dir, "comfort_perfect_3min_script.txt")
    with open(script_path, 'w', encoding='utf-8') as f:
        f.write(script_content)
    print(f"📝 스크립트 저장: {script_path}")
    
    # 3. TTS 생성
    audio_path, script = generator.generate_tts(script_content, "comfort_perfect_3min_narration")
