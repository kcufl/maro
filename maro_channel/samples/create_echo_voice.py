...
class EchoVoiceCreator:
    ...
    def generate_echo_tts(self, text):
        """echo 목소리로 TTS 음성 생성"""
        try:
            print(f"🎤 echo 목소리로 TTS 음성 생성 중...")
            print(f"📝 스크립트 길이: {len(text)}자")
            
            response = self.client.audio.speech.create(
                model="tts-1",
                voice="echo",  # 따뜻하고 친근한 목소리
                input=text,
                speed=1.0  # 자연스러운 속도
            )
            ...
            audio_path = os.path.join(self.output_dir, "echo_narration.mp3")
            
            with open(audio_path, "wb") as f:
                f.write(response.content)
            
            print(f"✅ echo TTS 음성 생성 완료: {audio_path}")
            return audio_path
    ...
    def create_background_music(self):
        ...
        bg_music_path = os.path.join(self.output_dir, "echo_bg_music.mp3")
    ...
    def merge_audio_simple(self, tts_path, bg_music_path):
        ...
        final_audio_path = os.path.join(self.output_dir, "echo_audio_with_bg.mp3")
    ...
    def create_final_video(self, video_path, audio_path):
        ...
        output_path = os.path.join(self.output_dir, "maro_echo_voice.mp4")
...
def main():
    print("🎤 maro 채널 echo 목소리 영상 생성 시작")
    print("=" * 60)
    
    creator = EchoVoiceCreator()
    ...
    # 2. echo 목소리로 TTS 생성
    tts_path = creator.generate_echo_tts(script)
    if not tts_path:
        print("❌ TTS 생성 실패")
        return
