#!/usr/bin/env python3
"""
maro 채널 대안 부드러운 목소리 영상 생성기
- 다른 TTS 목소리 시도 (shimmer, echo, fable, onyx)
- 배경음악 문제 해결
"""
...
class SoftVoiceAlternativeCreator:
    def __init__(self):
        # API 키 직접 설정 (임시)
    ...
    def generate_soft_tts(self, text, voice_name="shimmer"):
        """다른 부드러운 TTS 음성 생성"""
        try:
            print(f"🎤 {voice_name} 목소리로 TTS 음성 생성 중...")
            print(f"📝 스크립트 길이: {len(text)}자")
            
            response = self.client.audio.speech.create(
                model="tts-1",
                voice=voice_name,  # shimmer, echo, fable, onyx 중 선택
                input=text,
                speed=1.0  # 자연스러운 속도
            )
            ...
            audio_path = os.path.join(self.output_dir, f"soft_{voice_name}_narration.mp3")
            
            with open(audio_path, "wb") as f:
                f.write(response.content)
            
            print(f"✅ {voice_name} TTS 음성 생성 완료: {audio_path}")
            return audio_path
    ...
    def create_background_music(self):
        ...
        bg_music_path = os.path.join(self.output_dir, "soft_bg_music.mp3")
    ...
    def create_final_video(self, video_path, audio_path):
        ...
        output_path = os.path.join(self.output_dir, "maro_soft_voice_alternative.mp4")
...
def main():
    print("🎤 maro 채널 대안 부드러운 목소리 영상 생성 시작")
    print("=" * 60)
    
    creator = SoftVoiceAlternativeCreator()
    ...
    # 2. shimmer 목소리로 TTS 생성 (가장 부드러운 목소리)
    tts_path = creator.generate_soft_tts(script, "shimmer")
    if not tts_path:
        print("❌ TTS 생성 실패")
        return
